	...
	extends MySqlDatabase<PrismaMySqlQueryResultHKT, PrismaMySqlPreparedQueryHKT, Record<string, never>>
{
	...
	constructor(client: PrismaClient, logger: Logger | undefined, unrestrictedUpdates: boolean = false) {
		const dialect = new MySqlDialect({ unrestrictedUpdates });
...
export function drizzle(config: PrismaMySqlConfig = {}) {
	...
	return Prisma.defineExtension((client) => {
		return client.$extends({
			name: 'drizzle',
			client: {
				$drizzle: new PrismaMySqlDatabase(client, logger, config.unrestrictedUpdates),
			},
		});
	});
}
